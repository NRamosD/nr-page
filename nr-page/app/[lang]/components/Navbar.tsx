...
const NavBar = async (props : {lang:any}) => {
    ...
    return (
    <>
        <nav className='transition-all duration-1000 ease-out hidden md:block scale-75 hover:scale-90'>
            <ul className='inline-flex gap-5 md:gap-10 px-2 pb-2 pt-2 md:pb-2 w-full uppercase'>
                <li className='transition-all duration-500 border-2 px-5 py-1 hover:translate-y-1 hover:bg-slate-400 text-[10px] md:text-[12px] xl:text-base rounded-md'>
                    <a href='#start'>{intl.nav.Start}</a>
                </li>
                <li className='transition-all duration-500 border-2 px-5 py-1 hover:translate-y-1 hover:bg-slate-400 text-[10px] md:text-[12px] xl:text-base rounded-md'>
                    <a href='#summary'>{intl.nav.Summary}</a>
            ...
            <div className=' bottom-2 xl:top-2 right-2 text-left justify-center text-[.7rem] md:text-base rounded-md'>
                <h2>{intl.nav['turn-to']}</h2>
                <SwitchLang/>
            </div>
        </nav>
